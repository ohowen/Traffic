...
public class Traffic {
	...
	public long[] getAllGprs() {
		SharedPreferences preferences = context.getSharedPreferences(
		        "total_traffic", Context.MODE_PRIVATE);
		...
		if (TrafficStats.getMobileTxBytes() > a
		        || TrafficStats.getMobileTxBytes() == a) {
		...
		if (!now.equals(yet)) {
			if (getCurrentTime()[1] > preferences2.getInt("mouth", 2)
				...
				editor2.putLong("mouth_gprs_base",
				        gprs[2] + preferences2.getLong("mouth_gprs_base", 0));
			}
			...
			editor2.putLong("mouth_gprs_all",
			        preferences2.getLong("mouth_gprs_base", 0) + gprs[2]);
			editor2.commit();
		}
		Log.v("fuck",
		        getCurrentTime()[1] + "ss" + preferences2.getInt("mouth", 2)
		                + "aa" + getCurrentTime()[2] + "PP"
	...
	public long[] getAllWifi() {
		...
		if (TrafficStats.getTotalTxBytes() - TrafficStats.getMobileTxBytes() > e
		        || TrafficStats.getTotalTxBytes()
		                - TrafficStats.getMobileTxBytes() == e) {
			editor.putLong("wifi_send", TrafficStats.getTotalTxBytes()
			        - TrafficStats.getMobileTxBytes() - e + g);
			editor.putLong("wifi_rev", TrafficStats.getTotalRxBytes()
			        - TrafficStats.getMobileRxBytes() - f + h);
		} else {
			editor.putLong("wifi_send", TrafficStats.getTotalTxBytes()
			        - TrafficStats.getMobileTxBytes() + g);

			editor.putLong("wifi_rev", TrafficStats.getTotalRxBytes()
			        - TrafficStats.getMobileRxBytes() + h);
		}
